...
public class TransactionServiceTest {
    ...
    public void setUp() throws WalletException {
        ...
        transactionCredit = new Transaction(/*String.valueOf(globalIdCounter++),*/ typeDebit, 20, wallet1, null);
        transactionCredit.setId(5);
        transactionDebit = new Transaction(/*String.valueOf(globalIdCounter++),*/ typeCredit, 10, wallet2, 1L);
    ...
    public void testCreateTransaction_SuccessCredit() throws WalletException {
        int amount = 100;
        Mockito.when(walletService.addWalletAmount(wallet1.getId(), amount))
                .thenReturn(walletDtoConverter.convert(wallet1));
        Mockito.when(transactionRepository.save(Mockito.any(Transaction.class))).thenReturn(transactionCredit);
        int counter = globalIdCounter++;
        TransactionDTO found = transactionService.createTransactionAndChangeBalance(
                /*String.valueOf(counter),*/ typeCredit.getId(), amount, wallet1.getId(), 1L);
        assertNotNull(found);
    }
    ...
    public void testCreateTransaction_SuccessDebit() throws WalletException {
        int amount = -10;
        Mockito.when(walletService.deleteWalletAmount(wallet1.getId(), -1 * amount))
                .thenReturn(walletDtoConverter.convert(wallet1));
        Mockito.when(transactionRepository.save(Mockito.any(Transaction.class))).thenReturn(transactionCredit);
        int counter = globalIdCounter++;
        TransactionDTO found = transactionService.createTransactionAndChangeBalance(
